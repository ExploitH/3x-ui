#!/usr/bin/env python3
"""Add/rollback the HK2-backed US3 IPv4 edge DNS canary."""
...
ZONE = '427357.xyz'
NAME = 'edge-us3.427357.xyz'
IP = '141.11.148.116'
STATE = pathlib.Path('/www/projects/vpn-3xui-unification/audit/edge-us3-dns-canary-20260823.json')
...
def load_state() -> dict[str, Any] | None:
    ...
    try:
        ...
        raise RuntimeError('US3 DNS state is unreadable; refusing to continue') from exc
    if state.get('zone') != ZONE or state.get('name') != NAME or state.get('expected') != IP:
        raise RuntimeError('US3 DNS state identity mismatch; refusing to continue')
    created = state.get('created')
    if not isinstance(created, list) or len(created) != 1 or not created[0].get('id'):
        raise RuntimeError('US3 DNS state ownership is incomplete; refusing to continue')
    return state
...
def delete_owned(token: str, zone: str, record_id: str) -> bool:
    current = api(token, 'GET', f'/zones/{zone}/dns_records/{record_id}', allow_404=True)
    if current is None:
        return False
    if not owned_record(current):
        raise RuntimeError('US3 DNS record ownership changed or record is missing')
    ...
    if api(token, 'GET', f'/zones/{zone}/dns_records/{record_id}', allow_404=True) is not None:
        raise RuntimeError('US3 DNS record still exists after deletion')
    return True
    ...
    with LOCK.open('a+', encoding='utf-8') as lock_stream:
        ...
        if STATE.exists():
            ...
            if owned_record(api(token, 'GET', f'/zones/{zone}/dns_records/{record_id}', allow_404=True)):
                print(json.dumps({'status': 'EDGE_US3_DNS_CANARY_ALREADY_APPLIED', 'record': NAME}, sort_keys=True))
                return 0
            raise RuntimeError('US3 DNS state exists but its record is not owned/current; refusing apply')
        if records_for_name(token, zone):
            raise RuntimeError('existing DNS record(s) for edge-us3.427357.xyz; refusing to adopt or overwrite')
        created = api(token, 'POST', f'/zones/{zone}/dns_records', {
            'type': 'A', 'name': NAME, 'content': IP, 'ttl': 120,
            'proxied': False, 'comment': 'US3 IPv4 L4 edge canary',
        })
        if not owned_record(created):
            raise RuntimeError('Cloudflare returned a non-owned US3 DNS record')
        ...
        print(json.dumps({'status': 'EDGE_US3_DNS_CANARY_APPLIED', 'record': NAME, 'created_count': 1, 'state': str(STATE)}, sort_keys=True))
        return 0
...
def rollback(token: str, account: str) -> int:
    ...
    with LOCK.open('a+', encoding='utf-8') as lock_stream:
        ...
        remove_state()
        print(json.dumps({'status': 'EDGE_US3_DNS_CANARY_ROLLBACK_OK', 'deleted_count': int(deleted), 'state_reconciled': not deleted}, sort_keys=True))
        return 0
