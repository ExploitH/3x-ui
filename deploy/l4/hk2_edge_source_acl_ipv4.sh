...
TABLE=neko_hk2_edge_acl_ipv4
FAMILY=inet
RULES=/etc/nftables.d/neko-hk2-edge-acl-ipv4.nft
UNIT=/etc/systemd/system/neko-hk2-edge-acl-ipv4.service
STATE_DIR=/var/lib/neko-hk2-edge-acl-ipv4
STATE=$STATE_DIR/last-backup
FRONTEND_IP=91.229.132.66
PROXY_TCP_PORT=8881
PROXY_UDP_PORTS='8882,8883'
BACKUP=/root/neko-vpn-backup-$(date -u +%Y%m%dT%H%M%SZ)-hk2-edge-acl-ipv4
...
write_rules() {
  cat > "$RULES" <<EOF
# HK2 IPv4 source ACL: only the dedicated HK2 L4 edge may reach proxy ports.
table $FAMILY $TABLE {
    set trusted_hk1_ipv4 {
        type ipv4_addr
    ...
    chain input {
        ...
        ip saddr @trusted_hk1_ipv4 tcp dport $PROXY_TCP_PORT accept
        ip saddr @trusted_hk1_ipv4 udp dport { $PROXY_UDP_PORTS } accept
...
write_unit() {
  cat > "$UNIT" <<EOF
[Unit]
Description=Neko HK2 IPv4 source ACL for dedicated L4 edge
...
remove_own() {
  set +e
  systemctl stop neko-hk2-edge-acl-ipv4.service >/dev/null 2>&1 || true
  nft delete table "$FAMILY" "$TABLE" >/dev/null 2>&1 || true
  systemctl disable neko-hk2-edge-acl-ipv4.service >/dev/null 2>&1 || true
  rm -f "$RULES" "$UNIT"
...
apply() {
  ...
  systemctl enable neko-hk2-edge-acl-ipv4.service >/dev/null
  systemctl start neko-hk2-edge-acl-ipv4.service
...
rollback() {
  ...
  remove_own
  printf 'HK2_EDGE_ACL_IPV4_ROLLBACK_OK\n'
  printf 'backup=%s\n' "$BACKUP"
}

status() {
  nft list table "$FAMILY" "$TABLE"
  systemctl is-active neko-hk2-edge-acl-ipv4.service
}
