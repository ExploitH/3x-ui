...
BACKEND_IP=156.246.90.242
IFACE=eth0
TCP_VLESS_FRONT_PORT=40881
TCP_VLESS_BACKEND_PORT=8881
UDP_HY2_FRONT_PORT=40882
UDP_HY2_BACKEND_PORT=8882
UDP_TUIC_FRONT_PORT=40883
UDP_TUIC_BACKEND_PORT=8883
TCP_SS_FRONT_PORT=40885
UDP_SS_FRONT_PORT=40885
TCP_SS_BACKEND_PORT=8885
UDP_SS_BACKEND_PORT=8885
TCP_TROJAN_FRONT_PORT=40886
TCP_TROJAN_BACKEND_PORT=8886
TCP_H2_FRONT_PORT=40889
TCP_H2_BACKEND_PORT=8889
TCP_GRPC_FRONT_PORT=40890
TCP_GRPC_BACKEND_PORT=8890
TCP_ANYTLS_FRONT_PORT=40891
TCP_ANYTLS_BACKEND_PORT=8891
HELPER=/usr/local/sbin/neko-hk2-jp3-edge-forward
CONF=/etc/neko-hk2-jp3-edge-forward.conf
UNIT=/etc/systemd/system/neko-jp3-edge-forward.service
BACKUP=/root/neko-vpn-backup-$(date -u +%Y%m%dT%H%M%SZ)-hk2-jp3-edge-forward
DNAT_CHAIN=NEKO_JP3EDGE_DNAT
SNAT_CHAIN=NEKO_JP3EDGE_SNAT
FWD_CHAIN=NEKO_JP3EDGE_FWD
...
edge_enabled_before=$(systemctl is-enabled neko-jp3-edge-forward.service 2>/dev/null || true)
edge_active_before=$(systemctl is-active neko-jp3-edge-forward.service 2>/dev/null || true)
case "$edge_enabled_before:$edge_active_before" in
  enabled:*|masked:*|static:*|indirect:*|generated:*|transient:*|*:active)
    echo 'existing JP3 edge unit is enabled or active; refusing to overwrite' >&2
    exit 1
    ;;
esac
for path in "$HELPER" "$CONF" "$UNIT"; do
  if [ -e "$path" ] || [ -L "$path" ]; then
    echo "existing JP3 edge path found: $path; refusing to overwrite" >&2
    exit 1
  fi
done
edge_load_state=$(systemctl show -p LoadState --value neko-jp3-edge-forward.service 2>/dev/null || true)
case "$edge_load_state" in
  ''|not-found) ;;
  *) echo "existing JP3 edge unit is loaded ($edge_load_state); refusing to overwrite" >&2; exit 1 ;;
esac
if iptables -w 5 -t nat -nL "$DNAT_CHAIN" >/dev/null 2>&1 \
  ...
  echo 'existing JP3 edge iptables chain found; refusing to overwrite' >&2
  exit 1
fi
...
rollback() {
  set +e
  systemctl stop neko-jp3-edge-forward.service >/dev/null 2>&1 || true
  case "$edge_enabled_before" in
    enabled) systemctl enable neko-jp3-edge-forward.service >/dev/null 2>&1 || true ;;
    *) systemctl disable neko-jp3-edge-forward.service >/dev/null 2>&1 || true ;;
  esac
  cleanup_rules
  if ! verify_rules_absent; then
    echo 'JP3 edge rollback incomplete: iptables chains remain' >&2
    return 1
  fi
  rm -f "$HELPER" "$CONF" "$UNIT"
  if [ -e "$HELPER" ] || [ -e "$CONF" ] || [ -e "$UNIT" ] || [ -L "$HELPER" ] || [ -L "$CONF" ] || [ -L "$UNIT" ]; then
    echo 'JP3 edge rollback incomplete: generated paths remain' >&2
    return 1
  fi
  systemctl daemon-reload >/dev/null 2>&1 || true
  restore_service_state
...
cat > "$BACKUP/neko-hk2-jp3-edge-forward.generated" <<'EOF'
#!/bin/sh
set -eu
...
status_rules() {
  ...
  printf 'front=%s backend=%s ports=40881/tcp,40882/udp,40883/udp,40885/tcp+udp,40886/tcp,40889/tcp,40890/tcp,40891/tcp ip_forward=1\n' "$FRONT_IP" "$BACKEND_IP"
}
...
EOF
chmod 0755 "$BACKUP/neko-hk2-jp3-edge-forward.generated"
cat > "$CONF" <<EOF
FRONT_IP=$FRONT_IP
...
EOF
chmod 0600 "$CONF"
install -m 0755 "$BACKUP/neko-hk2-jp3-edge-forward.generated" "$HELPER"
cat > "$UNIT" <<'EOF'
[Unit]
Description=Neko HK2 to JP3 dedicated L4 edge forwarding canary
...
Type=oneshot
RemainAfterExit=yes
ExecStart=/usr/local/sbin/neko-hk2-jp3-edge-forward apply
ExecReload=/usr/local/sbin/neko-hk2-jp3-edge-forward apply
ExecStop=/usr/local/sbin/neko-hk2-jp3-edge-forward remove
...
systemctl enable neko-jp3-edge-forward.service >/dev/null
systemctl start neko-jp3-edge-forward.service
"$HELPER" status
...
trap - EXIT
rm -f "$BACKUP/neko-hk2-jp3-edge-forward.generated"
printf 'HK2_JP3_L4_FORWARD_CANARY_OK\n'
printf 'backup=%s\n' "$BACKUP"
printf 'service=%s\n' "$(systemctl is-active neko-jp3-edge-forward.service)"
