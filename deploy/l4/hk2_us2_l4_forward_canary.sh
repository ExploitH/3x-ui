...
BACKEND_IP=104.223.55.204
IFACE=eth0
TCP_VLESS_FRONT_PORT=41881
TCP_VLESS_BACKEND_PORT=8881
UDP_HY2_FRONT_PORT=41882
UDP_HY2_BACKEND_PORT=8882
UDP_TUIC_FRONT_PORT=41883
UDP_TUIC_BACKEND_PORT=8883
TCP_SS_FRONT_PORT=41885
UDP_SS_FRONT_PORT=41885
TCP_SS_BACKEND_PORT=8885
UDP_SS_BACKEND_PORT=8885
TCP_TROJAN_FRONT_PORT=41886
TCP_TROJAN_BACKEND_PORT=8886
TCP_H2_FRONT_PORT=41889
TCP_H2_BACKEND_PORT=8889
TCP_GRPC_FRONT_PORT=41890
TCP_GRPC_BACKEND_PORT=8890
TCP_ANYTLS_FRONT_PORT=41891
TCP_ANYTLS_BACKEND_PORT=8891
HELPER=/usr/local/sbin/neko-hk2-us2-edge-forward
CONF=/etc/neko-hk2-us2-edge-forward.conf
UNIT=/etc/systemd/system/neko-us2-edge-forward.service
BACKUP=/root/neko-vpn-backup-$(date -u +%Y%m%dT%H%M%SZ)-hk2-us2-edge-forward
DNAT_CHAIN=NEKO_US2EDGE_DNAT
SNAT_CHAIN=NEKO_US2EDGE_SNAT
FWD_CHAIN=NEKO_US2EDGE_FWD
CONFIG=/etc/sing-box/config.json
FRONT_PORT_REGEX='41881|41882|41883|41885|41886|41889|41890|41891'
...
if grep -Eq ":($FRONT_PORT_REGEX)([^0-9]|$)" <<<"$listeners"; then
  echo 'one or more US2 edge frontend ports are already listening; refusing to continue' >&2
  exit 1
fi
...
if grep -Eq -- "(^|[[:space:]])--dport ($FRONT_PORT_REGEX)([[:space:]]|$)" <<<"$iptables_preflight"; then
  echo 'one or more US2 edge frontend ports already exist in iptables; refusing to continue' >&2
  exit 1
fi
...
edge_enabled_before=$(systemctl is-enabled neko-us2-edge-forward.service 2>/dev/null || true)
edge_active_before=$(systemctl is-active neko-us2-edge-forward.service 2>/dev/null || true)
case "$edge_enabled_before:$edge_active_before" in
  enabled:*|masked:*|static:*|indirect:*|generated:*|transient:*|*:active)
    echo 'existing US2 edge unit is enabled or active; refusing to overwrite' >&2
    exit 1
    ;;
esac
for path in "$HELPER" "$CONF" "$UNIT"; do
  if [ -e "$path" ] || [ -L "$path" ]; then
    echo "existing US2 edge path found: $path; refusing to overwrite" >&2
    exit 1
  fi
done
edge_load_state=$(systemctl show -p LoadState --value neko-us2-edge-forward.service 2>/dev/null || true)
case "$edge_load_state" in
  ''|not-found) ;;
  *) echo "existing US2 edge unit is loaded ($edge_load_state); refusing to overwrite" >&2; exit 1 ;;
esac
if iptables -w 5 -t nat -nL "$DNAT_CHAIN" >/dev/null 2>&1 \
  ...
  echo 'existing US2 edge iptables chain found; refusing to overwrite' >&2
  exit 1
fi
mkdir -p "$BACKUP"
for path in "$HELPER" "$CONF" "$UNIT"; do
...
rollback() {
  set +e
  systemctl stop neko-us2-edge-forward.service >/dev/null 2>&1 || true
  case "$edge_enabled_before" in
    enabled) systemctl enable neko-us2-edge-forward.service >/dev/null 2>&1 || true ;;
    *) systemctl disable neko-us2-edge-forward.service >/dev/null 2>&1 || true ;;
  esac
  cleanup_rules
  if ! verify_rules_absent; then
    echo 'US2 edge rollback incomplete: iptables chains remain' >&2
    return 1
  fi
  rm -f "$HELPER" "$CONF" "$UNIT"
  if [ -e "$HELPER" ] || [ -e "$CONF" ] || [ -e "$UNIT" ] || [ -L "$HELPER" ] || [ -L "$CONF" ] || [ -L "$UNIT" ]; then
    echo 'US2 edge rollback incomplete: generated paths remain' >&2
    return 1
  fi
  systemctl daemon-reload >/dev/null 2>&1 || true
  restore_service_state
...
cat > "$BACKUP/neko-hk2-us2-edge-forward.generated" <<'EOF'
#!/bin/sh
set -eu
...
status_rules() {
  ...
  printf 'front=%s backend=%s ports=41881/tcp,41882/udp,41883/udp,41885/tcp+udp,41886/tcp,41889/tcp,41890/tcp,41891/tcp ip_forward=1\\n' "$FRONT_IP" "$BACKEND_IP"
}
...
EOF
chmod 0755 "$BACKUP/neko-hk2-us2-edge-forward.generated"
cat > "$CONF" <<EOF
FRONT_IP=$FRONT_IP
...
EOF
chmod 0600 "$CONF"
install -m 0755 "$BACKUP/neko-hk2-us2-edge-forward.generated" "$HELPER"
cat > "$UNIT" <<'EOF'
[Unit]
Description=Neko HK2 to US2 dedicated L4 edge forwarding canary
...
Type=oneshot
RemainAfterExit=yes
ExecStart=/usr/local/sbin/neko-hk2-us2-edge-forward apply
ExecReload=/usr/local/sbin/neko-hk2-us2-edge-forward apply
ExecStop=/usr/local/sbin/neko-hk2-us2-edge-forward remove
...
systemctl enable neko-us2-edge-forward.service >/dev/null
systemctl start neko-us2-edge-forward.service
"$HELPER" status
...
trap - EXIT
rm -f "$BACKUP/neko-hk2-us2-edge-forward.generated"
printf 'HK2_US2_L4_FORWARD_CANARY_OK\n'
printf 'backup=%s\n' "$BACKUP"
printf 'service=%s\n' "$(systemctl is-active neko-us2-edge-forward.service)"
