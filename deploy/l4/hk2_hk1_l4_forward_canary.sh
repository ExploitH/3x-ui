...
BACKEND_IP=91.229.132.66
IFACE=eth0
TCP_VLESS_FRONT_PORT=43881
TCP_VLESS_BACKEND_PORT=8881
UDP_HY2_FRONT_PORT=43882
UDP_HY2_BACKEND_PORT=8882
UDP_TUIC_FRONT_PORT=43883
UDP_TUIC_BACKEND_PORT=8883
TCP_TROJAN_FRONT_PORT=43886
TCP_TROJAN_BACKEND_PORT=8886
TCP_H2_FRONT_PORT=43889
TCP_H2_BACKEND_PORT=8889
TCP_GRPC_FRONT_PORT=43890
TCP_GRPC_BACKEND_PORT=8890
TCP_ANYTLS_FRONT_PORT=43891
TCP_ANYTLS_BACKEND_PORT=8891
UDP_RELAY_US2_HY2_FRONT_PORT=43892
UDP_RELAY_US2_HY2_BACKEND_PORT=28882
UDP_RELAY_US2_TUIC_FRONT_PORT=43893
UDP_RELAY_US2_TUIC_BACKEND_PORT=28883
UDP_RELAY_US3_HY2_FRONT_PORT=43894
UDP_RELAY_US3_HY2_BACKEND_PORT=28884
UDP_RELAY_US3_TUIC_FRONT_PORT=43895
UDP_RELAY_US3_TUIC_BACKEND_PORT=28885
TCP_RELAY_US2_REALITY_FRONT_PORT=43896
TCP_RELAY_US2_REALITY_BACKEND_PORT=28886
TCP_RELAY_US3_REALITY_FRONT_PORT=43897
TCP_RELAY_US3_REALITY_BACKEND_PORT=28887
HELPER=/usr/local/sbin/neko-hk2-hk1-edge-forward
CONF=/etc/neko-hk2-hk1-edge-forward.conf
UNIT=/etc/systemd/system/neko-hk1-edge-forward.service
BACKUP=/root/neko-vpn-backup-$(date -u +%Y%m%dT%H%M%SZ)-hk2-hk1-edge-forward
DNAT_CHAIN=NEKO_HK1EDGE_DNAT
SNAT_CHAIN=NEKO_HK1EDGE_SNAT
FWD_CHAIN=NEKO_HK1EDGE_FWD
CONFIG=/etc/sing-box/config.json
FRONT_PORT_REGEX='43881|43882|43883|43886|43889|43890|43891|43892|43893|43894|43895|43896|43897'
...
if grep -Eq ":($FRONT_PORT_REGEX)([^0-9]|$)" <<<"$listeners"; then
  echo 'one or more HK1 edge frontend ports are already listening; refusing to continue' >&2
  exit 1
fi
...
if grep -Eq -- "(^|[[:space:]])--dport ($FRONT_PORT_REGEX)([[:space:]]|$)" <<<"$iptables_preflight"; then
  echo 'one or more HK1 edge frontend ports already exist in iptables; refusing to continue' >&2
  exit 1
fi
...
edge_enabled_before=$(systemctl is-enabled neko-hk1-edge-forward.service 2>/dev/null || true)
edge_active_before=$(systemctl is-active neko-hk1-edge-forward.service 2>/dev/null || true)
case "$edge_enabled_before:$edge_active_before" in
  enabled:*|masked:*|static:*|indirect:*|generated:*|transient:*|*:active)
    echo 'existing HK1 edge unit is enabled or active; refusing to overwrite' >&2
    exit 1
    ;;
esac
for path in "$HELPER" "$CONF" "$UNIT"; do
  if [ -e "$path" ] || [ -L "$path" ]; then
    echo "existing HK1 edge path found: $path; refusing to overwrite" >&2
    exit 1
  fi
done
edge_load_state=$(systemctl show -p LoadState --value neko-hk1-edge-forward.service 2>/dev/null || true)
case "$edge_load_state" in
  ''|not-found) ;;
  *) echo "existing HK1 edge unit is loaded ($edge_load_state); refusing to overwrite" >&2; exit 1 ;;
esac
if iptables -w 5 -t nat -nL "$DNAT_CHAIN" >/dev/null 2>&1 \
  ...
  echo 'existing HK1 edge iptables chain found; refusing to overwrite' >&2
  exit 1
fi
mkdir -p "$BACKUP"
for path in "$HELPER" "$CONF" "$UNIT"; do
...
rollback() {
  set +e
  systemctl stop neko-hk1-edge-forward.service >/dev/null 2>&1 || true
  case "$edge_enabled_before" in
    enabled) systemctl enable neko-hk1-edge-forward.service >/dev/null 2>&1 || true ;;
    *) systemctl disable neko-hk1-edge-forward.service >/dev/null 2>&1 || true ;;
  esac
  cleanup_rules
  if ! verify_rules_absent; then
    echo 'HK1 edge rollback incomplete: iptables chains remain' >&2
    return 1
  fi
  rm -f "$HELPER" "$CONF" "$UNIT"
  if [ -e "$HELPER" ] || [ -e "$CONF" ] || [ -e "$UNIT" ] || [ -L "$HELPER" ] || [ -L "$CONF" ] || [ -L "$UNIT" ]; then
    echo 'HK1 edge rollback incomplete: generated paths remain' >&2
    return 1
  fi
  systemctl daemon-reload >/dev/null 2>&1 || true
  restore_service_state
...
cat > "$BACKUP/neko-hk2-hk1-edge-forward.generated" <<'EOF'
#!/bin/sh
set -eu
...
apply_rules() {
  ...
  add_pair udp "$UDP_RELAY_US2_HY2_FRONT_PORT" "$UDP_RELAY_US2_HY2_BACKEND_PORT"
  add_pair udp "$UDP_RELAY_US2_TUIC_FRONT_PORT" "$UDP_RELAY_US2_TUIC_BACKEND_PORT"
  add_pair udp "$UDP_RELAY_US3_HY2_FRONT_PORT" "$UDP_RELAY_US3_HY2_BACKEND_PORT"
  add_pair udp "$UDP_RELAY_US3_TUIC_FRONT_PORT" "$UDP_RELAY_US3_TUIC_BACKEND_PORT"
  add_pair tcp "$TCP_RELAY_US2_REALITY_FRONT_PORT" "$TCP_RELAY_US2_REALITY_BACKEND_PORT"
  add_pair tcp "$TCP_RELAY_US3_REALITY_FRONT_PORT" "$TCP_RELAY_US3_REALITY_BACKEND_PORT"
...
status_rules() {
  ...
  printf 'front=%s backend=%s ports=43881/tcp,43882/udp,43883/udp,43886/tcp,43889/tcp,43890/tcp,43891/tcp,43892/udp,43893/udp,43894/udp,43895/udp,43896/tcp,43897/tcp ip_forward=1\\n' "$FRONT_IP" "$BACKEND_IP"
}
...
EOF
chmod 0755 "$BACKUP/neko-hk2-hk1-edge-forward.generated"
cat > "$CONF" <<EOF
FRONT_IP=$FRONT_IP
...
UDP_RELAY_US2_HY2_FRONT_PORT=$UDP_RELAY_US2_HY2_FRONT_PORT
UDP_RELAY_US2_HY2_BACKEND_PORT=$UDP_RELAY_US2_HY2_BACKEND_PORT
UDP_RELAY_US2_TUIC_FRONT_PORT=$UDP_RELAY_US2_TUIC_FRONT_PORT
UDP_RELAY_US2_TUIC_BACKEND_PORT=$UDP_RELAY_US2_TUIC_BACKEND_PORT
UDP_RELAY_US3_HY2_FRONT_PORT=$UDP_RELAY_US3_HY2_FRONT_PORT
UDP_RELAY_US3_HY2_BACKEND_PORT=$UDP_RELAY_US3_HY2_BACKEND_PORT
UDP_RELAY_US3_TUIC_FRONT_PORT=$UDP_RELAY_US3_TUIC_FRONT_PORT
UDP_RELAY_US3_TUIC_BACKEND_PORT=$UDP_RELAY_US3_TUIC_BACKEND_PORT
TCP_RELAY_US2_REALITY_FRONT_PORT=$TCP_RELAY_US2_REALITY_FRONT_PORT
TCP_RELAY_US2_REALITY_BACKEND_PORT=$TCP_RELAY_US2_REALITY_BACKEND_PORT
TCP_RELAY_US3_REALITY_FRONT_PORT=$TCP_RELAY_US3_REALITY_FRONT_PORT
TCP_RELAY_US3_REALITY_BACKEND_PORT=$TCP_RELAY_US3_REALITY_BACKEND_PORT
DNAT_CHAIN=$DNAT_CHAIN
SNAT_CHAIN=$SNAT_CHAIN
FWD_CHAIN=$FWD_CHAIN
EOF
chmod 0600 "$CONF"
install -m 0755 "$BACKUP/neko-hk2-hk1-edge-forward.generated" "$HELPER"
cat > "$UNIT" <<'EOF'
[Unit]
Description=Neko HK2 to HK1 dedicated L4 edge forwarding canary
...
Type=oneshot
RemainAfterExit=yes
ExecStart=/usr/local/sbin/neko-hk2-hk1-edge-forward apply
ExecReload=/usr/local/sbin/neko-hk2-hk1-edge-forward apply
ExecStop=/usr/local/sbin/neko-hk2-hk1-edge-forward remove
...
systemctl enable neko-hk1-edge-forward.service >/dev/null
systemctl start neko-hk1-edge-forward.service
"$HELPER" status
...
trap - EXIT
rm -f "$BACKUP/neko-hk2-hk1-edge-forward.generated"
printf 'HK2_HK1_L4_FORWARD_CANARY_OK\n'
printf 'backup=%s\n' "$BACKUP"
printf 'service=%s\n' "$(systemctl is-active neko-hk1-edge-forward.service)"
