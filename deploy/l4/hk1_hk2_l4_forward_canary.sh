...
FRONT_IP=91.229.132.66
BACKEND_IP=141.11.148.116
IFACE=eth0
TCP_FRONT_PORT=38981
TCP_BACKEND_PORT=8881
UDP_HY2_FRONT_PORT=38982
UDP_HY2_BACKEND_PORT=8882
UDP_TUIC_FRONT_PORT=38983
UDP_TUIC_BACKEND_PORT=8883
HELPER=/usr/local/sbin/neko-hk3-edge-forward
CONF=/etc/neko-hk3-edge-forward.conf
UNIT=/etc/systemd/system/neko-hk2-edge-forward.service
BACKUP=/root/neko-vpn-backup-$(date -u +%Y%m%dT%H%M%SZ)-hk1-hk2-edge-forward
DNAT_CHAIN=NEKO_HK2EDGE_DNAT
SNAT_CHAIN=NEKO_HK2EDGE_SNAT
FWD_CHAIN=NEKO_HK2EDGE_FWD
...
edge_enabled_before=$(systemctl is-enabled neko-hk2-edge-forward.service 2>/dev/null || true)
edge_active_before=$(systemctl is-active neko-hk2-edge-forward.service 2>/dev/null || true)
...
config_hash_before=$(sha256sum /etc/sing-box/conf/*.json 2>/dev/null | sha256sum | awk '{print $1}' || true)
...
rollback() {
  set +e
  systemctl stop neko-hk2-edge-forward.service >/dev/null 2>&1 || true
  case "$edge_enabled_before" in
    enabled) systemctl enable neko-hk2-edge-forward.service >/dev/null 2>&1 || true ;;
    *) systemctl disable neko-hk2-edge-forward.service >/dev/null 2>&1 || true ;;
  esac
  cleanup_rules
  ...
  if [ -f "$BACKUP/neko-hk2-edge-forward.service.before" ]; then cp -a "$BACKUP/neko-hk2-edge-forward.service.before" "$UNIT"; else rm -f "$UNIT"; fi
  systemctl daemon-reload >/dev/null 2>&1 || true
  restore_service_state
...
systemctl enable neko-hk2-edge-forward.service >/dev/null
systemctl start neko-hk2-edge-forward.service
"$HELPER" status
...
config_hash_after=$(sha256sum /etc/sing-box/conf/*.json 2>/dev/null | sha256sum | awk '{print $1}' || true)
...
printf 'HK1_HK2_L4_FORWARD_CANARY_OK\n'
printf 'backup=%s\n' "$BACKUP"
printf 'service=%s\n' "$(systemctl is-active neko-hk2-edge-forward.service)"
