...
TABLE=neko_us2_edge_acl_ipv4
FAMILY=inet
RULES=/etc/nftables.d/neko-us2-edge-acl-ipv4.nft
UNIT=/etc/systemd/system/neko-us2-edge-acl-ipv4.service
DROPIN_DIR=/etc/systemd/system/sing-box.service.d
DROPIN=$DROPIN_DIR/90-neko-us2-edge-acl-ipv4.conf
DROPIN_DIR_WAS_PRESENT=0
[ -d "$DROPIN_DIR" ] && DROPIN_DIR_WAS_PRESENT=1
STATE_DIR=/var/lib/neko-us2-edge-acl-ipv4
...
BACKUP=/root/neko-vpn-backup-$(date -u +%Y%m%dT%H%M%SZ)-us2-edge-acl-ipv4
...
write_rules() {
  cat > "$RULES" <<EOF
# US2 IPv4 source ACL: only HK2 and existing trusted relay sources may reach direct proxy ports.
table $FAMILY $TABLE {
...
write_unit() {
  cat > "$UNIT" <<EOF
[Unit]
Description=Neko US2 IPv4 source ACL for HK2 and existing relay sources
...
write_dropin() {
  ...
  cat > "$DROPIN" <<EOF
[Unit]
Requires=neko-us2-edge-acl-ipv4.service
After=neko-us2-edge-acl-ipv4.service
EOF
}

remove_own() {
  set +e
  systemctl stop neko-us2-edge-acl-ipv4.service >/dev/null 2>&1 || true
  nft delete table "$FAMILY" "$TABLE" >/dev/null 2>&1 || true
  systemctl disable neko-us2-edge-acl-ipv4.service >/dev/null 2>&1 || true
...
apply() {
  ...
  systemctl enable neko-us2-edge-acl-ipv4.service >/dev/null
  systemctl start neko-us2-edge-acl-ipv4.service
  ...
  if ! grep -Fxq 'neko-us2-edge-acl-ipv4.service' <<<"$(tr ' ' '\n' <<<"$singbox_requires")"; then
    echo 'sing-box does not require the US2 ACL unit; refusing success' >&2
    exit 1
  fi

  trap - EXIT
  printf 'US2_EDGE_ACL_IPV4_APPLIED\n'
  printf 'table=%s %s\n' "$FAMILY" "$TABLE"
  printf 'trusted_relay_ipv4=%s\n' "$TRUSTED_RELAY_IPV4S"
  printf 'tcp_ports=%s\n' "$PROXY_TCP_PORTS"
  printf 'udp_ports=%s\n' "$PROXY_UDP_PORTS"
  printf 'service=%s\n' "$(service_state neko-us2-edge-acl-ipv4.service)"
...
rollback() {
  ...
  printf 'US2_EDGE_ACL_IPV4_ROLLBACK_OK\n'
  printf 'backup=%s\n' "$BACKUP"
}

status() {
  nft list table "$FAMILY" "$TABLE"
  systemctl is-active neko-us2-edge-acl-ipv4.service
}
